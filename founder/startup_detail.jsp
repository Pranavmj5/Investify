...
<body class="bg-background-light font-display text-text-main antialiased min-h-screen flex flex-col">
        ...
        <div class="mx-auto flex h-18 max-w-7xl items-center justify-between px-6 py-4 lg:px-8">
            ...
            <nav class="hidden md:flex items-center gap-8 pl-8 border-l border-border-subtle h-8">
                ...
                <a href="create_profile.jsp"
                    class="text-sm font-medium text-text-muted hover:text-primary transition-colors">My Startup</a>
                <a href="browse.jsp" class="text-sm font-semibold text-primary border-b-2 border-primary pb-0.5">Browse
                    Portal</a>
                <a href="messages.jsp"
                    class="text-sm font-medium text-text-muted hover:text-primary transition-colors">Messages</a>
            </nav>
                ...
                <div class="flex flex-col gap-6 fade-up">
                    ...
                    <div class="flex items-start gap-5">
                        <div
                            ...
                            <img id="startup-logo" alt="Startup logo" class="h-full w-full object-contain rounded-xl"
                                src="" />
                        </div>
                        <div>
                            <h1 class="text-3xl md:text-4xl font-bold tracking-tight" id="startup-name">Loading…</h1>
                ...
                <div class="relative fade-up stagger-2">
                    <div class="sticky top-24 flex flex-col gap-5">
                        ...
                        <div class="rounded-2xl border border-border-subtle bg-surface-light p-5 shadow-sm">
                            <h4 class="text-xs font-semibold uppercase tracking-wider text-text-muted mb-4">Lead Founder
                            </h4>
                            <div class="flex items-center gap-4">
    ...
    <div id="interest-modal" class="fixed inset-0 z-50 hidden flex items-center justify-center">
        ...
        <div class="relative z-10 w-full max-w-lg mx-4 bg-white rounded-2xl shadow-2xl p-8">
            ...
            <p class="text-text-muted text-sm mb-6">Let the startup know you're interested. Our team will follow up
                within 48 hours.</p>
            <form id="interest-form" onsubmit="submitInterest(event)" class="space-y-4">
                <div>
                ...
                <div class="flex items-start gap-2 text-sm text-text-muted">
                    ...
                    <label for="nda-check">I agree to the <a href="#" class="text-primary hover:underline">NDA terms</a>
    ...
    <footer class="border-t border-border-subtle bg-white py-10">
        <div class="mx-auto flex max-w-7xl flex-col items-center justify-between gap-6 px-6 lg:flex-row lg:px-8">
    ...
    <div>
      ...
      <div class="h-2 w-full rounded-full bg-gray-100"><div class="h-full rounded-full ${m.opacity} bar-fill" data-pct="${m.pct}"></div></div>
            ...
            // Documents
            const docs = [
                { name: "Pitch Deck Q3", icon: "picture_as_pdf", color: "text-red-500", locked: false },
                { name: "Financial Model", icon: "description", color: "text-blue-500", locked: false },
                { name: "Cap Table", icon: "table_chart", color: "text-green-500", locked: true },
            ];
            $("documents-list").innerHTML = docs.map(d => `
    <a href="#" class="flex items-center justify-between rounded-xl bg-background-light p-3 hover:bg-primary/5 group transition-colors">
    ...
    <div class="flex items-center justify-between py-2 border-b border-border-subtle last:border-0">
      ...
      <span class="text-sm font-semibold">${t.v}</span>
        ...
        function submitInterest(e) {
            e.preventDefault();
            $("interest-form").classList.add("hidden");
